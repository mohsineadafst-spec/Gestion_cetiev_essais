<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier l\'assignation #') }} {{ $demandeEssai->id }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" x-data="{ 
                    essaiOption: 'existant'
                }">
                    <form method="POST" action="{{ route('demande_essai.update', $demandeEssai) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select id="demande_id" name="demande_id"
                                ...
                                @foreach ($produits as $produit)
                                    <option value="{{ $produit->id }}" {{ old('demande_id', $demandeEssai->demande_id) == $produit->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="essai_id" name="essai_id"
                                ...
                                @foreach ($essais as $essai)
                                    <option value="{{ $essai->id }}" {{ old('essai_id', $demandeEssai->essai_id) == $essai->id ? 'selected' : '' }}>
                                        {{ $essai->nom_essai }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="laboratoire_id" name="laboratoire_id"
                                ...
                                @foreach ($laboratoires as $labo)
                                    <option value="{{ $labo->id }}" {{ old('laboratoire_id', $demandeEssai->laboratoire_id) == $labo->id ? 'selected' : '' }}>
                                        {{ $labo->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="statut" name="statut"
                                    ...
                                    required>
                                <option value="catalogué" {{ old('statut', $demandeEssai->statut) === 'catalogué' ? 'selected' : '' }}>Catalogué</option>
                                <option value="non catalogué" {{ old('statut', $demandeEssai->statut) === 'non catalogué' ? 'selected' : '' }}>Non catalogué</option>
                            </select>
                            @error('statut')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="description" name="description" rows="3"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $demandeEssai->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="informations_complementaires" name="informations_complementaires" rows="3"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('informations_complementaires') border-red-500 @enderror">{{ old('informations_complementaires', $demandeEssai->informations_complementaires) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" id="echantillons" name="echantillons" value="{{ old('echantillons', $demandeEssai->echantillons) }}"
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                            <p class="text-sm text-gray-600">
                                <strong>Créé le :</strong> {{ $demandeEssai->created_at->format('d/m/Y à H:i') }}<br>
                                <strong>Modifié le :</strong> {{ $demandeEssai->updated_at->format('d/m/Y à H:i') }}
                            </p>
                        </div>

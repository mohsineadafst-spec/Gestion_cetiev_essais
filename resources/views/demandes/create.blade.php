<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Nouvelle demande de faisabilité') }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('demandes.store') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="commentaires" name="commentaires" rows="4"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('commentaires') border-red-500 @enderror">{{ old('commentaires') }}</textarea>
                            @error('commentaires')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="date" id="datereception" name="datereception" value="{{ old('datereception') }}"
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" id="delai_prev" name="delai_prev" value="{{ old('delai_prev') }}" min="0"
                        ...
                        <div class="mb-6">
                            ...
                            <select id="laboratoire" name="laboratoire"
                                ...
                                @foreach ($poles as $pole)
                                    <option value="{{ $pole }}" {{ old('laboratoire') === $pole ? 'selected' : '' }}>
                                        {{ $pole }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="faisable" name="faisable"
                                ...
                                <option value="Oui" {{ old('faisable') === 'Oui' ? 'selected' : '' }}>Oui</option>
                                <option value="Non" {{ old('faisable') === 'Non' ? 'selected' : '' }}>Non</option>
                            </select>
                            @error('faisable')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror

<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2.5">
            ...
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Modifier demande confirmée #{{ $demandesConfirmee->id }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
                ...
                <form method="POST" action="{{ route('demandes_confirmees.update', $demandesConfirmee->id) }}" class="space-y-6" x-data="{ selectedDemande: null }">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="demande_essai_id" name="demande_essai_id" 
                            ...
                            @foreach($demandesEssaiCloturees as $demande)
                                <option value="{{ $demande->id }}" 
                                        ...
                                        {{ old('demande_essai_id', $demandesConfirmee->demande_essai_id) == $demande->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" id="client" name="client" 
                               :value="selectedDemande?.client || '{{ old('client', $demandesConfirmee->client) }}'"
                    ...
                    <div>
                        ...
                        <input type="date" id="date_reception" name="date_reception" value="{{ old('date_reception', $demandesConfirmee->date_reception->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="numero_bc" name="numero_bc" value="{{ old('numero_bc', $demandesConfirmee->numero_bc) }}"
                    ...
                    <div>
                        ...
                        <input type="date" id="date_reception_bc" name="date_reception_bc" value="{{ old('date_reception_bc', $demandesConfirmee->date_reception_bc->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select id="laboratoire_id" name="laboratoire_id"
                                :value="selectedDemande?.laboratoire || '{{ old('laboratoire_id', $demandesConfirmee->laboratoire_id) }}'"
                            ...
                            @foreach($laboratoires as $lab)
                                <option value="{{ $lab->id }}" {{ old('laboratoire_id', $demandesConfirmee->laboratoire_id) == $lab->id ? 'selected' : '' }}>
                                    {{ $lab->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="confirmation" name="confirmation"
                            ...
                            <option value="oui" {{ old('confirmation', $demandesConfirmee->confirmation) == 'oui' ? 'selected' : '' }}>Oui</option>
                            <option value="non" {{ old('confirmation', $demandesConfirmee->confirmation) == 'non' ? 'selected' : '' }}>Non</option>
                        </select>
                        @error('confirmation')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" id="code_rapport" name="code_rapport" value="{{ old('code_rapport', $demandesConfirmee->code_rapport) }}"

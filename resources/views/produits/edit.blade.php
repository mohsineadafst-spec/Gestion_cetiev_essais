                    ...
                    <form method="POST" action="{{ route('produits.update', $produit) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="text" id="client_name" name="client_name" value="{{ old('client_name', $produit->client_name) }}"
                            ...
                            <div class="mb-6">
                                ...
                                <select id="lab_id" name="lab_id"
                                    ...
                                    @foreach ($laboratoires as $labo)
                                        <option value="{{ $labo->id }}" {{ old('lab_id', $produit->lab_id) == $labo->id ? 'selected' : '' }}>
                                            {{ $labo->nom }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="date" id="date_reception" name="date_reception" value="{{ old('date_reception', $produit->date_reception->format('Y-m-d')) }}"
                            ...
                            <div class="mb-6">
                                ...
                                <input type="date" id="date_prevue" name="date_prevue" value="{{ old('date_prevue', $produit->date_prevue ? $produit->date_prevue->format('Y-m-d') : '') }}"
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" id="quantite" name="quantite" value="{{ old('quantite', $produit->quantite) }}" min="0"
                            ...
                            <div class="mb-6">
                                ...
                                <input type="text" id="marque" name="marque" value="{{ old('marque', $produit->marque) }}"
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" id="montant_ttc" name="montant_ttc" value="{{ old('montant_ttc', $produit->montant_ttc) }}" step="0.01" min="0"
                            ...
                            <div class="mb-6">
                                ...
                                <select id="statut_paiement" name="statut_paiement"
                                        ...
                                        required>
                                    <option value="pending" {{ old('statut_paiement', $produit->statut_paiement) === 'pending' ? 'selected' : '' }}>En attente</option>
                                    <option value="paid" {{ old('statut_paiement', $produit->statut_paiement) === 'paid' ? 'selected' : '' }}>Payé</option>
                                    <option value="cancelled" {{ old('statut_paiement', $produit->statut_paiement) === 'cancelled' ? 'selected' : '' }}>Annulé</option>
                                </select>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="mb-6">
                                ...
                                <select id="statut" name="statut"
                                        ...
                                        required>
                                    <option value="todo" {{ old('statut', $produit->statut) === 'todo' ? 'selected' : '' }}>À faire</option>
                                    <option value="in_progress" {{ old('statut', $produit->statut) === 'in_progress' ? 'selected' : '' }}>En cours</option>
                                    <option value="done" {{ old('statut', $produit->statut) === 'done' ? 'selected' : '' }}>Fait</option>
                                    <option value="cancelled" {{ old('statut', $produit->statut) === 'cancelled' ? 'selected' : '' }}>Annulé</option>
                                </select>
                                @error('statut')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="mb-6">
                                ...
                                <select id="resultat" name="resultat"
                                    ...
                                    <option value="conforme" {{ old('resultat', $produit->resultat) === 'conforme' ? 'selected' : '' }}>Conforme</option>
                                    <option value="non_conforme" {{ old('resultat', $produit->resultat) === 'non_conforme' ? 'selected' : '' }}>Non conforme</option>
                                    <option value="partiel" {{ old('resultat', $produit->resultat) === 'partiel' ? 'selected' : '' }}>Partiel</option>
                                </select>
                                @error('resultat')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="notes" name="notes" rows="3"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $produit->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            @if ($produit->url_rapport)
                                <div class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                                    <p class="text-sm text-blue-700">
                                        ...
                                        <a href="{{ $produit->url_rapport }}" target="_blank" class="underline hover:text-blue-900">
                                            Télécharger
                                        </a>
                                    </p>
                                </div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Formats acceptés: PDF, JPG, PNG, DOCX (Max: 5 Mo) - Laissez vide pour conserver le fichier actuel</p>
                            @error('rapport')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="details" name="details" rows="3"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('details') border-red-500 @enderror">{{ old('details', $produit->details) }}</textarea>
                            @error('details')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                            <p class="text-sm text-gray-600">
                                <strong>Créé le :</strong> {{ $produit->created_at->format('d/m/Y à H:i') }}<br>
                                <strong>Modifié le :</strong> {{ $produit->updated_at->format('d/m/Y à H:i') }}
                            </p>
                        </div>

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier l\'utilisateur : ') }} {{ $user->username }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('users.update', $user) }}">
                        ...
                        <div class="mb-6">
                            <label for="mdp" class="block text-sm font-medium text-gray-700 mb-2">
                                Nouveau mot de passe (laisser vide pour garder l'ancien) (minimum 8 caractères)
                            </label>
                                ...
                                @forelse($laboratoires as $labo)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="laboratoires[]" value="{{ $labo->id }}"
                                               ...
                                               {{ in_array($labo->id, old('laboratoires', $userLaboratoires ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                            <p class="text-sm text-gray-600">
                                ...
                                <strong>Modifié le :</strong> {{ $user->updated_at->format('d/m/Y à H:i') }}<br>
                                @if ($user->last_login)
                                    <strong>Dernière connexion :</strong> {{ $user->last_login->format('d/m/Y à H:i') }}
                                @else
